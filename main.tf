...
module "eks_cluster" {
  ...
  private_subnet_ids = module.vpc.private_subnet_ids  # Use private subnets in Node Group
...
module "rds" {
  ...
  eks_node_sg_id     = module.security_group.eks_node_sg_id  # Pass EKS node SG ID
  eks_security_group_id = module.eks_cluster.security_group_id  # Pass the EKS SG ID
